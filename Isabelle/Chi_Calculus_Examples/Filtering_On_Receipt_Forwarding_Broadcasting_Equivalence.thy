(*
  TODO:
    ...
    - Find a more elegant way for the following lemmas to play well with the `equivalence` method:
      `multi_receive_shortcut_redundancy`, `distributor_shortcut_redundancy` and
      `square_unidirectional_bridge_shortcut_redundancy`.

    - Move communication-related code to `Communication` theory.
...
theory Filtering_On_Receipt_Forwarding_Broadcasting_Equivalence
  imports
    Chi_Calculus.Core_Bisimilarities "HOL-Library.BNF_Corec" (* NOTE: From Communication.thy *)
    Chi_Calculus.Proper_Weak_Transition_System
    Chi_Calculus.Basic_Weak_Transition_System
begin

(* TODO: Discuss whether this is guard-related code is really required. If so, move it to
   `Processes.thy`. *)
lemma basic_guard_preservation:
  assumes "p \<sim>\<^sub>\<flat> q"
  shows "\<phi> ? p \<sim>\<^sub>\<flat> \<phi> ? q"
  using assms by simp

lemma basic_weak_guard_preservation:
  assumes "p \<approx>\<^sub>\<flat> q"
  shows "\<phi> ? p \<approx>\<^sub>\<flat> \<phi> ? q"
  using assms by simp

lemma proper_guard_preservation:
  assumes "p \<sim>\<^sub>\<sharp> q"
  shows "\<phi> ? p \<sim>\<^sub>\<sharp> \<phi> ? q"
  using assms by simp

lemma proper_weak_guard_preservation:
  assumes "p \<approx>\<^sub>\<sharp> q"
  shows "\<phi> ? p \<approx>\<^sub>\<sharp> \<phi> ? q"
  using assms by simp

context begin

private lift_definition
  basic_guard :: "[bool, basic_behavior] \<Rightarrow> basic_behavior"
  is guard
  using basic_guard_preservation .

private lift_definition
  basic_weak_guard :: "[bool, basic_weak_behavior] \<Rightarrow> basic_weak_behavior"
  is guard
  using basic_weak_guard_preservation .

private lift_definition
  proper_guard :: "[bool, proper_behavior] \<Rightarrow> proper_behavior"
  is guard
  using proper_guard_preservation .

private lift_definition
  proper_weak_guard :: "[bool, proper_weak_behavior] \<Rightarrow> proper_weak_behavior"
  is guard
  using proper_weak_guard_preservation .

lemmas [equivalence_transfer] =
  basic_guard.abs_eq
  basic_weak_guard.abs_eq
  proper_guard.abs_eq
  proper_weak_guard.abs_eq

end

(*** BEGIN Communication.thy ***)
...
abbreviation sidetrack_addition_diamond where
  "sidetrack_addition_diamond r\<^sub>0 s\<^sub>0 r\<^sub>1 s\<^sub>1 r\<^sub>2 s\<^sub>2 r\<^sub>3 s\<^sub>3 \<phi> \<equiv>
    \<nu> l\<^sub>0\<^sub>1 l\<^sub>0\<^sub>2 l\<^sub>1\<^sub>3 l\<^sub>2\<^sub>3 l\<^sub>3\<^sub>0. (
      \<currency>\<^sup>*l\<^sub>0\<^sub>1 \<parallel> \<currency>\<^sup>*l\<^sub>0\<^sub>2 \<parallel> \<currency>\<^sup>*l\<^sub>1\<^sub>3 \<parallel> \<currency>\<^sup>*l\<^sub>2\<^sub>3 \<parallel> \<currency>\<^sup>*l\<^sub>3\<^sub>0 \<parallel>
      ...
      \<comment> \<open>node 3\<close> \<nu> ib\<^sub>3 ob\<^sub>3. (
          s\<^sub>3 \<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3)
    )"

abbreviation buffered_relaying_diamond where
  "buffered_relaying_diamond r\<^sub>0 s\<^sub>0 r\<^sub>1 s\<^sub>1 r\<^sub>2 s\<^sub>2 r\<^sub>3 s\<^sub>3 \<phi> \<equiv>
    \<nu> l\<^sub>0\<^sub>1 l\<^sub>0\<^sub>2 l\<^sub>1\<^sub>3 l\<^sub>2\<^sub>3 l\<^sub>3\<^sub>0. (
      \<currency>\<^sup>*l\<^sub>0\<^sub>1 \<parallel> \<currency>\<^sup>*l\<^sub>0\<^sub>2 \<parallel> \<currency>\<^sup>*l\<^sub>1\<^sub>3 \<parallel> \<currency>\<^sup>*l\<^sub>2\<^sub>3 \<parallel> \<currency>\<^sup>*l\<^sub>3\<^sub>0 \<parallel>
      \<comment> \<open>node 0\<close> \<nu> ib\<^sub>0 ob\<^sub>0. (
          s\<^sub>0 \<rightarrow> ob\<^sub>0 \<parallel> ob\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2] \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0) \<parallel>
...
abbreviation send_collapsed_network where
  "send_collapsed_network r\<^sub>0 s\<^sub>0 r\<^sub>1 s\<^sub>1 r\<^sub>2 s\<^sub>2 r\<^sub>3 s\<^sub>3 \<phi> \<equiv>
    \<nu> l\<^sub>0\<^sub>1 l\<^sub>0\<^sub>2 l\<^sub>1\<^sub>3 l\<^sub>2\<^sub>3 l\<^sub>3\<^sub>0. (
      \<currency>\<^sup>*l\<^sub>0\<^sub>1 \<parallel> \<currency>\<^sup>*l\<^sub>0\<^sub>2 \<parallel> \<currency>\<^sup>*l\<^sub>1\<^sub>3 \<parallel> \<currency>\<^sup>*l\<^sub>2\<^sub>3 \<parallel> \<currency>\<^sup>*l\<^sub>3\<^sub>0 \<parallel>
      \<comment> \<open>node 0\<close> \<nu> ib\<^sub>0 ob\<^sub>0. (
        s\<^sub>0 \<rightarrow> ob\<^sub>0 \<parallel> ob\<^sub>0 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0) \<parallel>
      \<comment> \<open>node 1\<close> \<nu> ib\<^sub>1 ob\<^sub>1. (
        s\<^sub>1 \<rightarrow> ob\<^sub>1 \<parallel> ob\<^sub>1 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1) \<parallel>
      \<comment> \<open>node 2\<close> \<nu> ib\<^sub>2 ob\<^sub>2. (
        s\<^sub>2 \<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2) \<parallel>
      \<comment> \<open>node 3\<close> \<nu> ib\<^sub>3 ob\<^sub>3. (
        s\<^sub>3 \<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3) \<parallel>
      l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel>
      l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3
    )"

abbreviation buffered_broadcast where
  "buffered_broadcast r\<^sub>0 s\<^sub>0 r\<^sub>1 s\<^sub>1 r\<^sub>2 s\<^sub>2 r\<^sub>3 s\<^sub>3 \<phi> \<equiv>
    \<nu> m. (
      \<currency>\<^sup>*m \<parallel>
      \<comment> \<open>node 0\<close> \<nu> ib\<^sub>0 ob\<^sub>0. (s\<^sub>0 \<rightarrow> ob\<^sub>0 \<parallel> ob\<^sub>0 \<rightarrow> m \<parallel> m \<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0) \<parallel>
      \<comment> \<open>node 1\<close> \<nu> ib\<^sub>1 ob\<^sub>1. (s\<^sub>1 \<rightarrow> ob\<^sub>1 \<parallel> ob\<^sub>1 \<rightarrow> m \<parallel> m \<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1) \<parallel>
      \<comment> \<open>node 2\<close> \<nu> ib\<^sub>2 ob\<^sub>2. (s\<^sub>2 \<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<rightarrow> m \<parallel> m \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2) \<parallel>
      \<comment> \<open>node 3\<close> \<nu> ib\<^sub>3 ob\<^sub>3. (s\<^sub>3 \<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<rightarrow> m \<parallel> m \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3)
    )"
...
lemma diamond_sidetrack_addition:
  shows "
    ...
    ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3]
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>?r\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel>
    ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel>
    ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3"
proof -
  have "
    \<currency>\<^sup>?r\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel>
    ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3]
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel>
    \<Prod>b \<leftarrow> [r\<^sub>0]. \<currency>\<^sup>?b \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [ob\<^sub>0, r\<^sub>0]"
    unfolding distributor_def and general_parallel.simps using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel>
    \<Prod>b \<leftarrow> [r\<^sub>0]. \<currency>\<^sup>?b \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [ob\<^sub>0, r\<^sub>0] \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0"
    using sidetrack_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel> \<currency>\<^sup>?r\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel>
    ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> \<Prod>b \<leftarrow> [r\<^sub>1]. \<currency>\<^sup>?b \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [ob\<^sub>1, r\<^sub>1]"
    unfolding distributor_def and general_parallel.simps using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel> \<currency>\<^sup>?r\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel>
    ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> \<Prod>b \<leftarrow> [r\<^sub>1]. \<currency>\<^sup>?b \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [ob\<^sub>1, r\<^sub>1] \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1"
    using sidetrack_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>?r\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel> \<currency>\<^sup>?r\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel>
    ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> \<Prod>b \<leftarrow> [r\<^sub>2]. \<currency>\<^sup>?b \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [ob\<^sub>2, r\<^sub>2]"
    unfolding distributor_def and general_parallel.simps using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>?r\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel> \<currency>\<^sup>?r\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel>
    ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> \<Prod>b \<leftarrow> [r\<^sub>2]. \<currency>\<^sup>?b \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [ob\<^sub>2, r\<^sub>2] \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2"
    using sidetrack_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>?r\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel>
    ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> \<Prod>b \<leftarrow> [r\<^sub>3]. \<currency>\<^sup>?b \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [ob\<^sub>3, r\<^sub>3]"
    unfolding distributor_def and general_parallel.simps using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>?r\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel>
    ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> \<Prod>b \<leftarrow> [r\<^sub>3]. \<currency>\<^sup>?b \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [ob\<^sub>3, r\<^sub>3] \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3"
    using sidetrack_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>?r\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel>
    ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3"
    unfolding distributor_def and general_parallel.simps using natural_simps by equivalence
  finally show ?thesis .
qed
...
lemma two_target_distributor_split:
  shows "\<currency>\<^sup>+a \<parallel> \<currency>\<^sup>?b\<^sub>1 \<parallel> \<currency>\<^sup>?b\<^sub>2 \<parallel> a {\<phi>}\<Rightarrow> [b\<^sub>1, b\<^sub>2] \<approx>\<^sub>\<flat> \<currency>\<^sup>+a \<parallel> \<currency>\<^sup>?b\<^sub>1 \<parallel> \<currency>\<^sup>?b\<^sub>2 \<parallel> a {\<phi>}\<rightarrow> b\<^sub>1 \<parallel> a {\<phi>}\<rightarrow> b\<^sub>2"
proof -
  have "\<currency>\<^sup>+a \<parallel> \<currency>\<^sup>?b\<^sub>1 \<parallel> \<currency>\<^sup>?b\<^sub>2 \<parallel> a {\<phi>}\<Rightarrow> [b\<^sub>1, b\<^sub>2] \<approx>\<^sub>\<flat> \<currency>\<^sup>+a \<parallel> \<Prod>b \<leftarrow> [b\<^sub>1, b\<^sub>2]. \<currency>\<^sup>?b \<parallel> a {\<phi>}\<Rightarrow> [b\<^sub>1, b\<^sub>2]"
    using natural_simps unfolding general_parallel.simps by equivalence
  also have "\<dots> \<approx>\<^sub>\<flat> \<currency>\<^sup>+a \<parallel> \<Prod>b \<leftarrow> [b\<^sub>1, b\<^sub>2]. \<currency>\<^sup>?b \<parallel> \<Prod>b \<leftarrow> [b\<^sub>1, b\<^sub>2]. a {\<phi>}\<rightarrow> b"
    using distributor_split by equivalence
  also have "\<dots> \<approx>\<^sub>\<flat> \<currency>\<^sup>+a \<parallel> \<currency>\<^sup>?b\<^sub>1 \<parallel> \<currency>\<^sup>?b\<^sub>2 \<parallel> a {\<phi>}\<rightarrow> b\<^sub>1 \<parallel> a {\<phi>}\<rightarrow> b\<^sub>2"
    using natural_simps unfolding general_parallel.simps by equivalence
  finally show ?thesis .
qed
...
lemma square_unidirectional_bridge_shortcut_redundancy:
  assumes "\<chi> = \<phi> \<sqinter> \<psi> \<sqinter> \<theta>"
  shows "a {\<phi>}\<rightarrow> b \<parallel> b {\<psi>}\<rightarrow> c \<parallel> c {\<theta>}\<rightarrow> d \<parallel> a {\<chi>}\<rightarrow> d \<approx>\<^sub>\<flat> a {\<phi>}\<rightarrow> b \<parallel> b {\<psi>}\<rightarrow> c \<parallel> c {\<theta>}\<rightarrow> d"
proof -
  have "a {\<phi>}\<rightarrow> b \<parallel> b {\<psi>}\<rightarrow> c \<parallel> c {\<theta>}\<rightarrow> d \<approx>\<^sub>\<flat> c {\<theta>}\<rightarrow> d \<parallel> a {\<phi>}\<rightarrow> b \<parallel> b {\<psi>}\<rightarrow> c"
    using natural_simps by equivalence
  also have "\<dots> \<approx>\<^sub>\<flat> c {\<theta>}\<rightarrow> d \<parallel> a {\<phi>}\<rightarrow> b \<parallel> b {\<psi>}\<rightarrow> c \<parallel> a {\<phi> \<sqinter> \<psi>}\<rightarrow> c"
    using unidirectional_bridge_shortcut_redundancy by equivalence
  also have "\<dots> \<approx>\<^sub>\<flat> a {\<phi>}\<rightarrow> b \<parallel> b {\<psi>}\<rightarrow> c \<parallel> a {\<phi> \<sqinter> \<psi>}\<rightarrow> c \<parallel> c {\<theta>}\<rightarrow> d"
    using natural_simps by equivalence
  also have "\<dots> \<approx>\<^sub>\<flat> a {\<phi>}\<rightarrow> b \<parallel> b {\<psi>}\<rightarrow> c \<parallel> a {\<phi> \<sqinter> \<psi>}\<rightarrow> c \<parallel> c {\<theta>}\<rightarrow> d \<parallel> a {\<phi> \<sqinter> \<psi> \<sqinter> \<theta>}\<rightarrow> d"
    using unidirectional_bridge_shortcut_redundancy by equivalence
  also have "\<dots> \<approx>\<^sub>\<flat> c {\<theta>}\<rightarrow> d \<parallel> a {\<phi> \<sqinter> \<psi> \<sqinter> \<theta>}\<rightarrow> d \<parallel> a {\<phi>}\<rightarrow> b \<parallel> b {\<psi>}\<rightarrow> c \<parallel> a {\<phi> \<sqinter> \<psi>}\<rightarrow> c"
    using natural_simps by equivalence
  also have "\<dots> \<approx>\<^sub>\<flat> c {\<theta>}\<rightarrow> d \<parallel> a {\<phi> \<sqinter> \<psi> \<sqinter> \<theta>}\<rightarrow> d \<parallel> a {\<phi>}\<rightarrow> b \<parallel> b {\<psi>}\<rightarrow> c"
    using unidirectional_bridge_shortcut_redundancy by equivalence
  finally show ?thesis
    using natural_simps and assms(1) by equivalence
qed
...
lemma diamond_relaying_addition:
  shows "
    ...
    \<comment> \<open>node 0\<close> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> ob\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2] \<parallel>
    \<comment> \<open>node 1\<close> l\<^sub>0\<^sub>1 \<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> ob\<^sub>1 \<Rightarrow> [l\<^sub>1\<^sub>3] \<parallel>
    \<comment> \<open>node 2\<close> l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
    \<comment> \<open>node 3\<close> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>+l\<^sub>3\<^sub>0 \<parallel> \<currency>\<^sup>?l\<^sub>0\<^sub>1 \<parallel> \<currency>\<^sup>?l\<^sub>0\<^sub>2 \<parallel>
    \<comment> \<open>node 0\<close> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> ob\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2] \<parallel>
    \<comment> \<open>node 1\<close> l\<^sub>0\<^sub>1 \<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> ob\<^sub>1 \<Rightarrow> [l\<^sub>1\<^sub>3] \<parallel>
    \<comment> \<open>node 2\<close> l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
    \<comment> \<open>node 3\<close> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0"
    (is "?lhs \<approx>\<^sub>\<flat> ?rhs")
proof -
  ...
  have "
    ...
    \<comment> \<open>node 2\<close> l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
    \<comment> \<open>node 3\<close> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel>
    \<comment> \<open>node 0\<close> ob\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2] \<parallel> l\<^sub>3\<^sub>0 {\<top>}\<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0"
  ...
  also have "
    ...
    \<comment> \<open>node 2\<close> l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
    \<comment> \<open>node 3\<close> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel>
    \<comment> \<open>node 0\<close> ob\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2] \<parallel> l\<^sub>3\<^sub>0 {\<top>}\<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0"
   using unidirectional_bridge_shortcut_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    ...
    \<comment> \<open>node 2\<close> l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
    \<comment> \<open>node 3\<close> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel>
    \<comment> \<open>node 0\<close> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> ob\<^sub>0 {\<top>}\<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2]"
    unfolding unidirectional_bridge_def using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    ...
    \<comment> \<open>node 2\<close> l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
    \<comment> \<open>node 3\<close> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel>
    \<comment> \<open>node 0\<close> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> ob\<^sub>0 {\<top>}\<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2] \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2]"
  ...
  also have "
    ...
    \<comment> \<open>node 2\<close> l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
    \<comment> \<open>node 3\<close> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel>
    \<comment> \<open>node 0\<close> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> ob\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2] \<parallel>
    \<currency>\<^sup>+l\<^sub>3\<^sub>0 \<parallel> \<currency>\<^sup>?l\<^sub>0\<^sub>1 \<parallel> \<currency>\<^sup>?l\<^sub>0\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2]"
    using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    ...
    \<comment> \<open>node 3\<close> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel>
    \<comment> \<open>node 0\<close> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> ob\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2] \<parallel>
    \<currency>\<^sup>+l\<^sub>3\<^sub>0 \<parallel> \<currency>\<^sup>?l\<^sub>0\<^sub>1 \<parallel> \<currency>\<^sup>?l\<^sub>0\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2"
  ...
  also have "
    ...
    \<comment> \<open>node 2\<close> l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
    \<comment> \<open>node 3\<close> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel>
    \<comment> \<open>node 0\<close> ob\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2] \<parallel> l\<^sub>3\<^sub>0 {\<top>}\<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0"
    unfolding unidirectional_bridge_def using natural_simps by equivalence
  also have "
    \<dots>
  ...
  also have "
    ...
    \<comment> \<open>node 3\<close> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel>
    \<comment> \<open>node 1\<close> l\<^sub>0\<^sub>1 {\<top>}\<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> ob\<^sub>1 {\<top>}\<rightarrow> l\<^sub>1\<^sub>3"
    unfolding unidirectional_bridge_def using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    ...
    \<comment> \<open>node 3\<close> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel>
    \<comment> \<open>node 1\<close> l\<^sub>0\<^sub>1 {\<top>}\<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> ob\<^sub>1 {\<top>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3"
  ...
  also have "
    ...
    \<comment> \<open>node 1\<close> l\<^sub>0\<^sub>1 \<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> ob\<^sub>1 \<Rightarrow> [l\<^sub>1\<^sub>3] \<parallel>
    \<comment> \<open>node 3\<close> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel>
    \<comment> \<open>node 2\<close> l\<^sub>0\<^sub>2 {\<top>}\<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 {\<top>}\<rightarrow> l\<^sub>2\<^sub>3"
    unfolding unidirectional_bridge_def using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    ...
    \<comment> \<open>node 3\<close> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel>
    \<comment> \<open>node 2\<close> l\<^sub>0\<^sub>2 {\<top>}\<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 {\<top>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3"
    using square_unidirectional_bridge_shortcut_redundancy by equivalence
  ...
  also have "
    ...
    \<comment> \<open>node 1\<close> l\<^sub>0\<^sub>1 \<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> ob\<^sub>1 \<Rightarrow> [l\<^sub>1\<^sub>3] \<parallel>
    \<comment> \<open>node 2\<close> l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel>
    \<comment> \<open>node 3\<close> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<top>}\<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 {\<top>}\<rightarrow> l\<^sub>3\<^sub>0"
    unfolding unidirectional_bridge_def using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    ...
    \<comment> \<open>node 2\<close> l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel>
    \<comment> \<open>node 3\<close> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<top>}\<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 {\<top>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0"
    using square_unidirectional_bridge_shortcut_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    ...
    \<comment> \<open>node 1\<close> l\<^sub>0\<^sub>1 \<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> ob\<^sub>1 \<Rightarrow> [l\<^sub>1\<^sub>3] \<parallel>
    \<comment> \<open>node 2\<close> l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    \<comment> \<open>node 3\<close> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> l\<^sub>2\<^sub>3 {\<top>}\<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 {\<top>}\<rightarrow> l\<^sub>3\<^sub>0"
    unfolding unidirectional_bridge_def using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    ...
    \<comment> \<open>node 2\<close> l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    \<comment> \<open>node 3\<close> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> l\<^sub>2\<^sub>3 {\<top>}\<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 {\<top>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0"
    using square_unidirectional_bridge_shortcut_redundancy by equivalence
...
lemma two_channel_distributor_switch:
  shows "
    \<currency>\<^sup>*a \<parallel> b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> d \<Rightarrow> [b, c]
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>*a \<parallel> b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> d \<rightarrow> a"
proof -
  have "
    \<currency>\<^sup>*a \<parallel> b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> d \<Rightarrow> [b, c]
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>*a \<parallel> b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> d \<triangleright>\<^sup>\<infinity> x. \<Prod>b\<leftarrow>[b, c]. \<top> x ? b \<triangleleft> x"
    unfolding unidirectional_bridge_def and distributor_def by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>*a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> d \<triangleright>\<^sup>\<infinity> x. (\<top> x ? b \<triangleleft> x \<parallel> \<top> x ? c \<triangleleft> x)"
    using natural_simps unfolding general_parallel.simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>*a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> d \<triangleright>\<^sup>\<infinity> x. (b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> \<top> x ? b \<triangleleft> x \<parallel> \<top> x ? c \<triangleleft> x)"
  ...
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>*a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> d \<triangleright>\<^sup>\<infinity> x. ((b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> \<top> x ? a \<triangleleft> x) \<parallel> \<top> x ? c \<triangleleft> x)"
  ...
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>*a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> d \<triangleright>\<^sup>\<infinity> x. (\<top> x ? a \<triangleleft> x \<parallel> \<top> x ? c \<triangleleft> x)"
    using inner_bidirectional_bridge_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>*a \<parallel> b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> d \<triangleright>\<^sup>\<infinity> x. (\<top> x ? a \<triangleleft> x \<parallel> \<top> x ? c \<triangleleft> x)"
    using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>*a \<parallel> b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> d \<triangleright>\<^sup>\<infinity> x. (c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> \<top> x ? a \<triangleleft> x \<parallel> \<top> x ? c \<triangleleft> x)"
    using inner_bidirectional_bridge_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>*a \<parallel> b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> d \<triangleright>\<^sup>\<infinity> x. ((c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> \<top> x ? c \<triangleleft> x) \<parallel> \<top> x ? a \<triangleleft> x)"
    using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>*a \<parallel> b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> d \<triangleright>\<^sup>\<infinity> x. ((c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> \<top> x ? a \<triangleleft> x) \<parallel> \<top> x ? a \<triangleleft> x)"
    using send_channel_switch
    by (blast intro:
      basic_weak_parallel_preservation_right
      basic_weak_multi_receive_preservation
  ...
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>*a \<parallel> b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> d \<triangleright>\<^sup>\<infinity> x. (\<top> x ? a \<triangleleft> x \<parallel> \<top> x ? a \<triangleleft> x)"
    using inner_bidirectional_bridge_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> \<currency>\<^sup>*a \<parallel> d \<triangleright>\<^sup>\<infinity> x. (\<top> x ? a \<triangleleft> x \<parallel> \<top> x ? a \<triangleleft> x)"
    using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> \<currency>\<^sup>*a \<parallel> d \<triangleright>\<^sup>\<infinity> x. (\<currency>\<^sup>*a \<parallel> \<top> x ? a \<triangleleft> x \<parallel> \<top> x ? a \<triangleleft> x)"
    using inner_duploss_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> \<currency>\<^sup>*a \<parallel> d \<triangleright>\<^sup>\<infinity> x. (\<currency>\<^sup>*a \<parallel> a \<triangleleft> x \<parallel> a \<triangleleft> x)"
    by simp
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> \<currency>\<^sup>*a \<parallel> d \<triangleright>\<^sup>\<infinity> x. (\<currency>\<^sup>*a \<parallel> a \<triangleleft> x)"
    using send_idempotency_under_duploss by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> \<currency>\<^sup>*a \<parallel> d \<triangleright>\<^sup>\<infinity> x. a \<triangleleft> x"
    using inner_duploss_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>*a \<parallel> b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> d \<triangleright>\<^sup>\<infinity> x. (a \<triangleleft> x \<parallel> \<zero>)"
    using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    \<currency>\<^sup>*a \<parallel> b {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> c {\<phi>}\<leftrightarrow>{\<psi>} a \<parallel> d \<triangleright>\<^sup>\<infinity> x. \<Prod>b\<leftarrow>[a]. \<top> x ? b \<triangleleft> x"
    by simp
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
...
lemma send_collapse:
  shows "
    l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>1 \<Rightarrow> [l\<^sub>1\<^sub>3] \<parallel>
    l\<^sub>2\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
    a\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel>
    \<currency>\<^sup>*l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2]
    \<approx>\<^sub>\<flat>
    l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>1 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    l\<^sub>2\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>2 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    a\<^sub>3 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    \<currency>\<^sup>*l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>0 \<rightarrow> l\<^sub>3\<^sub>0"
proof -
  have "
    l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>1 \<Rightarrow> [l\<^sub>1\<^sub>3] \<parallel>
    l\<^sub>2\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
    ...
    \<currency>\<^sup>*l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2]
    \<approx>\<^sub>\<flat>
    (l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>1 {\<top>}\<rightarrow> l\<^sub>1\<^sub>3) \<parallel>
    l\<^sub>2\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>2 \<rightarrow> l\<^sub>2\<^sub>3 \<parallel>
    a\<^sub>3 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    \<currency>\<^sup>*l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2]"
    unfolding unidirectional_bridge_def using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    (l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>1 {\<top>}\<rightarrow> l\<^sub>3\<^sub>0) \<parallel>
    l\<^sub>2\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>2 \<rightarrow> l\<^sub>2\<^sub>3 \<parallel>
    a\<^sub>3 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    \<currency>\<^sup>*l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2]"
  ...
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    (l\<^sub>2\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>2 {\<top>}\<rightarrow> l\<^sub>2\<^sub>3) \<parallel>
    l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>1 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    a\<^sub>3 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    \<currency>\<^sup>*l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2]"
    unfolding unidirectional_bridge_def using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<flat>
    (l\<^sub>2\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>2 {\<top>}\<rightarrow> l\<^sub>3\<^sub>0) \<parallel>
    l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>1 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    a\<^sub>3 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    \<currency>\<^sup>*l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> a\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2]"
    using unidirectional_bridge_target_switch by (rule basic_weak_parallel_preservation_left)
...
lemma links_disconnect:
  shows "
    l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel>
    l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel>
    l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3
    \<approx>\<^sub>\<sharp>
    l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3"
proof -
  ...
  have "
    ...
    l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3
    \<approx>\<^sub>\<sharp>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel>
    l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel>
    l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2"
    unfolding bidirectional_bridge_def using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel>
    l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel>
    l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2"
    using unidirectional_bridge_shortcut_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel>
    l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1"
    using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel>
    l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1"
    using unidirectional_bridge_shortcut_redundancy by equivalence
  (* Disconnect l\<^sub>0\<^sub>1 and l\<^sub>1\<^sub>3. *)
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel>
    l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3"
    unfolding bidirectional_bridge_def using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
  ...
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel>
    l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel>
    l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1"
    using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel>
    l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1"
    using unidirectional_bridge_shortcut_redundancy by equivalence
  (* Disconnect l\<^sub>0\<^sub>1 and l\<^sub>2\<^sub>3. *)
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3"
    unfolding bidirectional_bridge_def using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3"
    using unidirectional_bridge_shortcut_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel>
    l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel>
    l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1"
    using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel>
    l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1"
    using unidirectional_bridge_shortcut_redundancy by equivalence
  (* Disconnect l\<^sub>0\<^sub>2 and l\<^sub>1\<^sub>3. *)
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel>
    l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel>
    l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3"
    unfolding bidirectional_bridge_def using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel>
    l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3"
    using unidirectional_bridge_shortcut_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel>
    l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2"
    using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel>
    l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2"
    using unidirectional_bridge_shortcut_redundancy by equivalence
  (* Disconnect l\<^sub>0\<^sub>2 and l\<^sub>2\<^sub>3. *)
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3"
    unfolding bidirectional_bridge_def using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3"
    using unidirectional_bridge_shortcut_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2"
    using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
    l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2"
    using unidirectional_bridge_shortcut_redundancy by equivalence
  (* Disconnect l\<^sub>1\<^sub>3 and l\<^sub>2\<^sub>3. *)
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel>
    l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3"
    unfolding bidirectional_bridge_def using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel>
    l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3"
    using unidirectional_bridge_shortcut_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel>
    l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3"
    using natural_simps by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel>
    l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3"
    using unidirectional_bridge_shortcut_redundancy by equivalence
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
...
lemma links_remove:
  shows "
    \<nu> l\<^sub>0\<^sub>1. (\<currency>\<^sup>*l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<psi>} l\<^sub>0\<^sub>1) \<parallel>
    \<nu> l\<^sub>0\<^sub>2. (\<currency>\<^sup>*l\<^sub>0\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<psi>} l\<^sub>0\<^sub>2) \<parallel>
    \<nu> l\<^sub>1\<^sub>3. (\<currency>\<^sup>*l\<^sub>1\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<psi>} l\<^sub>1\<^sub>3) \<parallel>
    \<nu> l\<^sub>2\<^sub>3. (\<currency>\<^sup>*l\<^sub>2\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<psi>} l\<^sub>2\<^sub>3)
    \<approx>\<^sub>\<sharp>
...
proof -
  have "
    ...
    \<currency>\<^sup>?r\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> \<langle>0\<rangle> \<nu> l\<^sub>0\<^sub>1. \<langle>1\<rangle> \<nu> l\<^sub>0\<^sub>2. \<langle>2\<rangle> \<nu> l\<^sub>1\<^sub>3. \<langle>3\<rangle> \<nu> l\<^sub>2\<^sub>3. \<langle>4\<rangle> \<nu> l\<^sub>3\<^sub>0. (
      \<currency>\<^sup>*l\<^sub>0\<^sub>1 \<parallel> \<currency>\<^sup>*l\<^sub>0\<^sub>2 \<parallel> \<currency>\<^sup>*l\<^sub>1\<^sub>3 \<parallel> \<currency>\<^sup>*l\<^sub>2\<^sub>3 \<parallel> \<currency>\<^sup>*l\<^sub>3\<^sub>0 \<parallel>
      \<langle>5\<rangle> \<nu> ib\<^sub>0. \<langle>6\<rangle> \<nu> ob\<^sub>0. (s\<^sub>0 \<rightarrow> ob\<^sub>0 \<parallel> ob\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2] \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0]) \<parallel>
      \<langle>7\<rangle> \<nu> ib\<^sub>1. \<langle>8\<rangle> \<nu> ob\<^sub>1. (s\<^sub>1 \<rightarrow> ob\<^sub>1 \<parallel> ob\<^sub>1 \<Rightarrow> [l\<^sub>1\<^sub>3] \<parallel> l\<^sub>0\<^sub>1 \<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1]) \<parallel>
      \<langle>9\<rangle> \<nu> ib\<^sub>2. \<langle>10\<rangle> \<nu> ob\<^sub>2. (s\<^sub>2 \<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel> l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2]) \<parallel>
      \<langle>11\<rangle> \<nu> ib\<^sub>3. \<langle>12\<rangle> \<nu> ob\<^sub>3. (s\<^sub>3 \<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3])
    )"
    unfolding tagged_new_channel_def ..
  also have "
    \<dots>
  ...
  also have "
    ...
    \<langle>0\<rangle> \<nu> l\<^sub>0\<^sub>1. \<langle>1\<rangle> \<nu> l\<^sub>0\<^sub>2. \<langle>2\<rangle> \<nu> l\<^sub>1\<^sub>3. \<langle>3\<rangle> \<nu> l\<^sub>2\<^sub>3. \<langle>4\<rangle> \<nu> l\<^sub>3\<^sub>0. (
      ...
      (s\<^sub>2 \<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel> l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2) \<parallel>
      (s\<^sub>3 \<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3) \<parallel>
      \<currency>\<^sup>?r\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel>
      ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel>
      ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3
    ))"
  ...
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    \<currency>\<^sup>?r\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> sidetrack_addition_diamond r\<^sub>0 s\<^sub>0 r\<^sub>1 s\<^sub>1 r\<^sub>2 s\<^sub>2 r\<^sub>3 s\<^sub>3 \<phi>"
    unfolding tagged_new_channel_def ..
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
  ...
  also have "
    ...
    \<currency>\<^sup>?r\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> \<langle>0\<rangle> \<nu> l\<^sub>0\<^sub>1. \<langle>1\<rangle> \<nu> l\<^sub>0\<^sub>2. \<langle>2\<rangle> \<nu> l\<^sub>1\<^sub>3. \<langle>3\<rangle> \<nu> l\<^sub>2\<^sub>3. \<langle>4\<rangle> \<nu> l\<^sub>3\<^sub>0. (
      \<currency>\<^sup>+l\<^sub>0\<^sub>1 \<parallel> \<currency>\<^sup>+l\<^sub>0\<^sub>2 \<parallel> \<currency>\<^sup>*l\<^sub>1\<^sub>3 \<parallel> \<currency>\<^sup>*l\<^sub>2\<^sub>3 \<parallel> \<currency>\<^sup>?l\<^sub>3\<^sub>0 \<parallel>
      \<langle>5\<rangle> \<nu> ib\<^sub>0. \<langle>6\<rangle> \<nu> ob\<^sub>0. \<langle>7\<rangle> \<nu> ib\<^sub>1. \<langle>8\<rangle> \<nu> ob\<^sub>1. \<langle>9\<rangle> \<nu> ib\<^sub>2. \<langle>10\<rangle> \<nu> ob\<^sub>2. \<langle>11\<rangle> \<nu> ib\<^sub>3. \<langle>12\<rangle> \<nu> ob\<^sub>3. (
        s\<^sub>0 \<rightarrow> ob\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel>
        s\<^sub>1 \<rightarrow> ob\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel>
        s\<^sub>2 \<rightarrow> ob\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel>
        s\<^sub>3 \<rightarrow> ob\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel>
        \<currency>\<^sup>+l\<^sub>3\<^sub>0 \<parallel> \<currency>\<^sup>?l\<^sub>0\<^sub>1 \<parallel> \<currency>\<^sup>?l\<^sub>0\<^sub>2 \<parallel>
        l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel> ob\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2] \<parallel>
        l\<^sub>0\<^sub>1 \<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> ob\<^sub>1 \<Rightarrow> [l\<^sub>1\<^sub>3] \<parallel>
        l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
        l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3
      )
    )"
    unfolding duploss_def using natural_simps by equivalence
  also have "
    ...
    \<currency>\<^sup>?r\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> \<langle>0\<rangle> \<nu> l\<^sub>0\<^sub>1. \<langle>1\<rangle> \<nu> l\<^sub>0\<^sub>2. \<langle>2\<rangle> \<nu> l\<^sub>1\<^sub>3. \<langle>3\<rangle> \<nu> l\<^sub>2\<^sub>3. \<langle>4\<rangle> \<nu> l\<^sub>3\<^sub>0. (
      ...
      \<langle>5\<rangle> \<nu> ib\<^sub>0. \<langle>6\<rangle> \<nu> ob\<^sub>0. \<langle>7\<rangle> \<nu> ib\<^sub>1. \<langle>8\<rangle> \<nu> ob\<^sub>1. \<langle>9\<rangle> \<nu> ib\<^sub>2. \<langle>10\<rangle> \<nu> ob\<^sub>2. \<langle>11\<rangle> \<nu> ib\<^sub>3. \<langle>12\<rangle> \<nu> ob\<^sub>3. (
        ...
        l\<^sub>0\<^sub>1 \<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel> ob\<^sub>1 \<Rightarrow> [l\<^sub>1\<^sub>3] \<parallel>
        l\<^sub>0\<^sub>2 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
        l\<^sub>1\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> l\<^sub>2\<^sub>3 \<rightarrow> ib\<^sub>3 \<parallel>
        l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<rightarrow> l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<rightarrow> l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<rightarrow> l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<rightarrow> l\<^sub>3\<^sub>0
      )
    )"
  ...
  also have "
    ...
    \<currency>\<^sup>?r\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> \<langle>0\<rangle> \<nu> l\<^sub>0\<^sub>1. \<langle>1\<rangle> \<nu> l\<^sub>0\<^sub>2. \<langle>2\<rangle> \<nu> l\<^sub>1\<^sub>3. \<langle>3\<rangle> \<nu> l\<^sub>2\<^sub>3. \<langle>4\<rangle> \<nu> l\<^sub>3\<^sub>0. (
      ...
      \<langle>5\<rangle> \<nu> ib\<^sub>0. \<langle>6\<rangle> \<nu> ob\<^sub>0. \<langle>7\<rangle> \<nu> ib\<^sub>1. \<langle>8\<rangle> \<nu> ob\<^sub>1. \<langle>9\<rangle> \<nu> ib\<^sub>2. \<langle>10\<rangle> \<nu> ob\<^sub>2. \<langle>11\<rangle> \<nu> ib\<^sub>3. \<langle>12\<rangle> \<nu> ob\<^sub>3. (
        s\<^sub>0 \<rightarrow> ob\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0 \<parallel>
        s\<^sub>1 \<rightarrow> ob\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1 \<parallel>
        s\<^sub>2 \<rightarrow> ob\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2 \<parallel>
        s\<^sub>3 \<rightarrow> ob\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel>
        l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel>
        l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> ob\<^sub>1 \<Rightarrow> [l\<^sub>1\<^sub>3] \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> ob\<^sub>2 \<Rightarrow> [l\<^sub>2\<^sub>3] \<parallel>
        ob\<^sub>3 \<Rightarrow> [l\<^sub>3\<^sub>0] \<parallel> \<currency>\<^sup>*l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> ob\<^sub>0 \<Rightarrow> [l\<^sub>0\<^sub>1, l\<^sub>0\<^sub>2]
      )
    )"
    using natural_simps by equivalence
  also have "
    ...
    \<currency>\<^sup>?r\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> \<langle>0\<rangle> \<nu> l\<^sub>0\<^sub>1. \<langle>1\<rangle> \<nu> l\<^sub>0\<^sub>2. \<langle>2\<rangle> \<nu> l\<^sub>1\<^sub>3. \<langle>3\<rangle> \<nu> l\<^sub>2\<^sub>3. \<langle>4\<rangle> \<nu> l\<^sub>3\<^sub>0. (
      ...
      \<langle>5\<rangle> \<nu> ib\<^sub>0. \<langle>6\<rangle> \<nu> ob\<^sub>0. \<langle>7\<rangle> \<nu> ib\<^sub>1. \<langle>8\<rangle> \<nu> ob\<^sub>1. \<langle>9\<rangle> \<nu> ib\<^sub>2. \<langle>10\<rangle> \<nu> ob\<^sub>2. \<langle>11\<rangle> \<nu> ib\<^sub>3. \<langle>12\<rangle> \<nu> ob\<^sub>3. (
        ...
        s\<^sub>3 \<rightarrow> ob\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3 \<parallel>
        l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>2 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel>
        l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3 \<parallel> l\<^sub>1\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> ob\<^sub>1 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>2\<^sub>3 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> ob\<^sub>2 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> ob\<^sub>3 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel>
        \<currency>\<^sup>*l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>1 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> l\<^sub>0\<^sub>2 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>3\<^sub>0 \<parallel> ob\<^sub>0 \<rightarrow> l\<^sub>3\<^sub>0
      )
    )"
  ...
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    \<currency>\<^sup>?r\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> send_collapsed_network r\<^sub>0 s\<^sub>0 r\<^sub>1 s\<^sub>1 r\<^sub>2 s\<^sub>2 r\<^sub>3 s\<^sub>3 \<phi>"
    unfolding tagged_new_channel_def ..
  also have "
    \<dots>
    \<approx>\<^sub>\<sharp>
    \<currency>\<^sup>?r\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> \<langle>0\<rangle> \<nu> l\<^sub>0\<^sub>1. \<langle>1\<rangle> \<nu> l\<^sub>0\<^sub>2. \<langle>2\<rangle> \<nu> l\<^sub>1\<^sub>3. \<langle>3\<rangle> \<nu> l\<^sub>2\<^sub>3. \<langle>4\<rangle> \<nu> l\<^sub>3\<^sub>0. (
      \<currency>\<^sup>*l\<^sub>0\<^sub>1 \<parallel> \<currency>\<^sup>*l\<^sub>0\<^sub>2 \<parallel> \<currency>\<^sup>*l\<^sub>1\<^sub>3 \<parallel> \<currency>\<^sup>*l\<^sub>2\<^sub>3 \<parallel> \<currency>\<^sup>*l\<^sub>3\<^sub>0 \<parallel>
      \<nu> ib\<^sub>0 ob\<^sub>0. (s\<^sub>0 \<rightarrow> ob\<^sub>0 \<parallel> ob\<^sub>0 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0) \<parallel>
      \<nu> ib\<^sub>1 ob\<^sub>1. (s\<^sub>1 \<rightarrow> ob\<^sub>1 \<parallel> ob\<^sub>1 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1) \<parallel>
      \<nu> ib\<^sub>2 ob\<^sub>2. (s\<^sub>2 \<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2) \<parallel>
      \<nu> ib\<^sub>3 ob\<^sub>3. (s\<^sub>3 \<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3) \<parallel>
      l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3
    )"
  ...
  also have "
    ...
    \<currency>\<^sup>?r\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> \<langle>4\<rangle> \<nu> l\<^sub>3\<^sub>0. (
      ...
      \<langle>0\<rangle> \<nu> l\<^sub>0\<^sub>1. (\<currency>\<^sup>*l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>1) \<parallel>
      \<langle>1\<rangle> \<nu> l\<^sub>0\<^sub>2. (\<currency>\<^sup>*l\<^sub>0\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>2) \<parallel>
      \<langle>2\<rangle> \<nu> l\<^sub>1\<^sub>3. (\<currency>\<^sup>*l\<^sub>1\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3) \<parallel>
      \<langle>3\<rangle> \<nu> l\<^sub>2\<^sub>3. (\<currency>\<^sup>*l\<^sub>2\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3) \<parallel>
      \<nu> ib\<^sub>0 ob\<^sub>0. (s\<^sub>0 \<rightarrow> ob\<^sub>0 \<parallel> ob\<^sub>0 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0) \<parallel>
      \<nu> ib\<^sub>1 ob\<^sub>1. (s\<^sub>1 \<rightarrow> ob\<^sub>1 \<parallel> ob\<^sub>1 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1) \<parallel>
      \<nu> ib\<^sub>2 ob\<^sub>2. (s\<^sub>2 \<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2) \<parallel>
      \<nu> ib\<^sub>3 ob\<^sub>3. (s\<^sub>3 \<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3)
    )"
    using natural_simps by equivalence
  also have "
    \<dots>
    ...
    \<currency>\<^sup>?r\<^sub>0 \<parallel> \<currency>\<^sup>?r\<^sub>1 \<parallel> \<currency>\<^sup>?r\<^sub>2 \<parallel> \<currency>\<^sup>?r\<^sub>3 \<parallel> \<nu> l\<^sub>3\<^sub>0. (
      \<currency>\<^sup>*l\<^sub>3\<^sub>0 \<parallel> (
      \<nu> l\<^sub>0\<^sub>1. (\<currency>\<^sup>*l\<^sub>0\<^sub>1 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>1) \<parallel>
      \<nu> l\<^sub>0\<^sub>2. (\<currency>\<^sup>*l\<^sub>0\<^sub>2 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>0\<^sub>2) \<parallel>
      \<nu> l\<^sub>1\<^sub>3. (\<currency>\<^sup>*l\<^sub>1\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>1\<^sub>3) \<parallel>
      \<nu> l\<^sub>2\<^sub>3. (\<currency>\<^sup>*l\<^sub>2\<^sub>3 \<parallel> l\<^sub>3\<^sub>0 {\<phi>}\<leftrightarrow>{\<phi>} l\<^sub>2\<^sub>3)) \<parallel>
      \<nu> ib\<^sub>0 ob\<^sub>0. (s\<^sub>0 \<rightarrow> ob\<^sub>0 \<parallel> ob\<^sub>0 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>0 \<parallel> ib\<^sub>0 {\<phi>}\<Rightarrow> [r\<^sub>0, ob\<^sub>0] \<parallel> ib\<^sub>0 {\<phi>}\<rightarrow> ob\<^sub>0) \<parallel>
      \<nu> ib\<^sub>1 ob\<^sub>1. (s\<^sub>1 \<rightarrow> ob\<^sub>1 \<parallel> ob\<^sub>1 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>1 \<parallel> ib\<^sub>1 {\<phi>}\<Rightarrow> [r\<^sub>1, ob\<^sub>1] \<parallel> ib\<^sub>1 {\<phi>}\<rightarrow> ob\<^sub>1) \<parallel>
      \<nu> ib\<^sub>2 ob\<^sub>2. (s\<^sub>2 \<rightarrow> ob\<^sub>2 \<parallel> ob\<^sub>2 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>2 \<parallel> ib\<^sub>2 {\<phi>}\<Rightarrow> [r\<^sub>2, ob\<^sub>2] \<parallel> ib\<^sub>2 {\<phi>}\<rightarrow> ob\<^sub>2) \<parallel>
      \<nu> ib\<^sub>3 ob\<^sub>3. (s\<^sub>3 \<rightarrow> ob\<^sub>3 \<parallel> ob\<^sub>3 \<rightarrow> l\<^sub>3\<^sub>0 \<parallel> l\<^sub>3\<^sub>0 \<rightarrow> ib\<^sub>3 \<parallel> ib\<^sub>3 {\<phi>}\<Rightarrow> [r\<^sub>3, ob\<^sub>3] \<parallel> ib\<^sub>3 {\<phi>}\<rightarrow> ob\<^sub>3)
