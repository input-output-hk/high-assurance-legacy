...
method equivalence = (
  ...
  ((match premises in prem [thin]: "_ \<Longrightarrow> _ \<Longrightarrow> _" \<Rightarrow> \<open>insert prem [uncurry]\<close>)+)?,
        ...
        inclusion [thin]: "\<X> \<le> \<Y>" (cut) for \<X> :: "['a, 'a] \<Rightarrow> bool" and \<Y> \<Rightarrow> \<open>
          ...
          match conclusion in
            "\<Y> _ _" \<Rightarrow> \<open>
              ...
              match premises in
                conditional_equivalences [thin]: "_ \<Longrightarrow> \<X> _ _" (multi) \<Rightarrow> \<open>
  ...
  ((match premises in prem [thin]: "_ &&& _ \<Longrightarrow> _" \<Rightarrow> \<open>insert prem [curry]\<close>)+)?,
