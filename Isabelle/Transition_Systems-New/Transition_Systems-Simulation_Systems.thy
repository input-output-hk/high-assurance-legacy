...
text \<open>
  ...
  \<open>(\<rightharpoonup>)\<lparr>\<alpha>\<rparr> = (=) \<and> (\<rightharpoondown>)\<lparr>\<alpha>\<rparr> = (=)\<close> and we have \<open>(=) = (\<rightharpoonup>)\<lparr>\<alpha>\<rparr>\<^bsup>0\<^esup> \<and> (=) = (\<rightharpoonup>)\<lparr>\<alpha>\<rparr>\<^bsup>0\<^esup>\<close>. This is made
