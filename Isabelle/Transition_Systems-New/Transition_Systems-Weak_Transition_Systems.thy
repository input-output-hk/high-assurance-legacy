...
private lemma mixed_bisimulation_is_multi_silent_simulation:
  assumes "mixed_sim K"
...
next
  ...
  proof (cases "\<alpha> = \<tau>")
    case True
    then show ?thesis
      using mixed_bisimulation_is_multi_silent_simulation [OF \<open>mixed_sim K\<close>] by simp
  next
    case False
    ...
    also have "\<dots> \<le> (\<rightarrow>\<lparr>\<tau>\<rparr>)\<^sup>*\<^sup>* OO K\<inverse>\<inverse> OO (\<rightarrow>\<lparr>\<alpha>\<rparr>) OO (\<rightarrow>\<lparr>\<tau>\<rparr>)\<^sup>*\<^sup>*"
      using mixed_bisimulation_is_multi_silent_simulation [OF \<open>mixed_sim K\<close>] by blast
...
definition
  pre_bisimilarity_chain :: "('p relation \<Rightarrow> 'p relation) \<Rightarrow> ('p relation \<Rightarrow> 'p relation)"
  (\<open>\<langle>[\<sim>] \<frown>\<rangle>\<close>)
where
  [simp]: "\<langle>[\<sim>] \<frown>\<rangle> \<F> = [\<sim>] \<frown> \<F>"

lemma pre_bisimilarity_chain_is_mixed_respectful [respectful]:
  assumes "mixed.respectful \<F>"
  shows "mixed.respectful (\<langle>[\<sim>] \<frown>\<rangle> \<F>)"
proof -
  write mixed.unilateral_progression (infix "\<hookrightarrow>" 50)
  write mixed.shortcut_progression (infix "\<leadsto>" 50)
  have "\<langle>[\<sim>] \<frown>\<rangle> \<F> K \<leadsto> \<langle>[\<sim>] \<frown>\<rangle> \<F> L" if "K \<leadsto> L" for K and L
  proof -
...
definition
  post_bisimilarity_chain :: "('p relation \<Rightarrow> 'p relation) \<Rightarrow> ('p relation \<Rightarrow> 'p relation)"
  (\<open>(_ \<langle>\<frown> [\<sim>]\<rangle>)\<close> [900] 900)
where
  [simp]: "\<F> \<langle>\<frown> [\<sim>]\<rangle> = \<F> \<frown> [\<sim>]"

lemma post_bisimilarity_chain_is_mixed_respectful [respectful]:
  assumes "mixed.respectful \<F>"
  shows "mixed.respectful (\<F> \<langle>\<frown> [\<sim>]\<rangle>)"
proof -
  have "\<F> \<langle>\<frown> [\<sim>]\<rangle> = (\<langle>[\<sim>] \<frown>\<rangle> \<F>\<^sup>\<dagger>)\<^sup>\<dagger>"
    by (rule ext) (auto intro: bisimilarity_symmetry_rule)
  moreover from \<open>mixed.respectful \<F>\<close> have "mixed.respectful (\<langle>[\<sim>] \<frown>\<rangle> \<F>\<^sup>\<dagger>)\<^sup>\<dagger>"
    by respectful
